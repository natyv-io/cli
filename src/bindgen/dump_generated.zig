...
pub fn main(init: std.process.Init) !void {
    ...
    // UTF-16 command-line string itself, which std.mem.span-style
    // indexing can't even typecheck against).
    var arg_iter = try std.process.Args.Iterator.initAllocator(init.minimal.args, allocator);
    defer arg_iter.deinit();
    _ = arg_iter.skip();
    const zig_out_path = arg_iter.next() orelse {
        std.debug.print("usage: dump_generated <zig-out-path> <go-out-path>\n", .{});
        return error.BadArgs;
    };
    const go_out_path = arg_iter.next() orelse {
        std.debug.print("usage: dump_generated <zig-out-path> <go-out-path>\n", .{});
        return error.BadArgs;
    };
