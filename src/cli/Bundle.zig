...
pub fn run(allocator: std.mem.Allocator, io: Io, natyv_core_src: []const u8, wasm_path: []const u8, config_path: []const u8, dist_dir: Io.Dir, output_name: []const u8, has_bindings: bool, binding_include_dirs: []const u8, binding_lib_dirs: []const u8, binding_link: []const u8, binding_zig_deps: []const u8, binding_vendor_c_files: []const u8, has_textures: bool, has_window_style: bool, has_app_font: bool, has_tray_icon: bool, sqlite_enabled: bool, bundle_id: []const u8, icon_path: ?[]const u8, target: ?[]const u8, target_os: TargetOs, linux_package: ?[]const u8, ca_certs_json: []const u8, cache_dir: []const u8, optimize_flag: []const u8) !Result {
    ...
    if (has_app_font) try argv.append(allocator, "-Dapp-font=true");
    if (has_tray_icon) try argv.append(allocator, "-Dtray-icon=true");
...
test "a NATYV_CORE_SRC that doesn't exist is a clear error" {
    ...
    const result = try run(std.testing.allocator, io, "/definitely/not/a/real/path", "app.wasm", "conf.natyv.json", tmp.dir, "myapp", false, "", "", "", "", "", false, false, false, false, true, "dev.natyv.myapp", null, null, .macos, null, "", "/tmp/natyv-test-cache", "-Doptimize=Debug");
...
test "a NATYV_CORE_SRC with no build.zig is a clear error" {
    ...
    const result = try run(std.testing.allocator, io, abs_path, "app.wasm", "conf.natyv.json", tmp.dir, "myapp", false, "", "", "", "", "", false, false, false, false, true, "dev.natyv.myapp", null, null, .macos, null, "", "/tmp/natyv-test-cache", "-Doptimize=Debug");
    defer if (result.err) |e| std.testing.allocator.free(e.message);
    try std.testing.expect(!result.ok);
...
test "a missing compiled wasm file is a clear error" {
    ...
    const result = try run(std.testing.allocator, io, abs_path, "/definitely/not/a/real/wasm/path.wasm", "conf.natyv.json", tmp.dir, "myapp", false, "", "", "", "", "", false, false, false, false, true, "dev.natyv.myapp", null, null, .macos, null, "", "/tmp/natyv-test-cache", "-Doptimize=Debug");
    defer if (result.err) |e| std.testing.allocator.free(e.message);
    try std.testing.expect(!result.ok);
...
test "a missing config file is a clear error" {
    ...
    const result = try run(std.testing.allocator, io, abs_path, wasm_abs, "/definitely/not/a/real/conf.natyv.json", tmp.dir, "myapp", false, "", "", "", "", "", false, false, false, false, true, "dev.natyv.myapp", null, null, .macos, null, "", "/tmp/natyv-test-cache", "-Doptimize=Debug");
    defer if (result.err) |e| std.testing.allocator.free(e.message);
    try std.testing.expect(!result.ok);
