...
pub const Outcome = struct {
    ...
    /// True only when the stylesheet declared the reserved `window` block
    /// and it was staged into natyv-core. `cli/main.zig`'s `.build` case
    /// reads this to decide whether to pass `-Dwindow-style=true`, exactly
    /// as `has_textures` drives `-Dhas-textures`. False and zero-cost for
    /// any app whose `.ntss` has no `window` block, or that has no `.ntss`
    /// at all.
    has_window_style: bool = false,
    /// True when an app-wide font and/or point size was staged into
    /// natyv-core; drives `-Dapp-font=true`, same shape as the two flags
    /// above.
    has_app_font: bool = false,
    /// True when conf.natyv.json's `icon` was staged into natyv-core for the
    /// system tray; drives `-Dtray-icon=true`, same shape as the three flags
    /// above. False and zero-cost for any app that configured no icon --
    /// `SDL_CreateTray` accepts a null icon, so a tray still works without
    /// one, it just has nothing to draw.
...
/// Stages the reserved `window` block into natyv-core as a tiny generated
/// Zig file, the same way `stageTextureAssets` stages texture bytes plus a
/// `TextureAssetsGenerated.zig` manifest.
///
/// This is the established channel for `.ntss` data that natyv-core itself
/// consumes rather than the guest: the window background is what SDL
/// clears the renderer to before any widget draws, so it cannot travel
/// through `styletokens_generated.go` (which only ever reaches the guest).
///
/// Writes nothing and returns false when there is no `window` block, so
/// `build.zig` keeps using `WindowStyleAbsent.zig` and an app that never
/// styles its window pays nothing.
/// Stages the app-wide default font into natyv-core: copies the real font
/// file out of the app's `assets/` directory and emits an
/// `AppFontGenerated.zig` that `@embedFile`s it, the same shape
/// `stageTextureAssets` uses for texture bytes.
///
/// Resolves the two sources here rather than in natyv-core, because only
/// this side can stage a file at all: the `.ntss` `font` block wins over
/// conf.natyv.json's `ui.font`/`ui.font_size`, matching how the `window`
/// block wins over `ui.background_color`. Each field resolves
/// independently, so a stylesheet naming only a file still picks up a
/// size from config.
///
/// A size with no file is legitimate (natyv's bundled Inter at a
/// different point size), so this stages whenever either resolves.
fn stageAppFont(allocator: std.mem.Allocator, io: Io, assets_dir: ?Io.Dir, font: ?Resolver.ResolvedFont, config_font: ?[]const u8, config_font_size: ?f32, natyv_core_src: []const u8) !struct { has_app_font: bool, err: ?PrepareError } {
    const file = if (font) |f| f.file orelse config_font else config_font;
    const size = if (font) |f| f.size orelse config_font_size else config_font_size;
    if (file == null and size == null) return .{ .has_app_font = false, .err = null };

    var core_dir = std.Io.Dir.cwd().openDir(io, natyv_core_src, .{}) catch |e| {
        return .{ .has_app_font = false, .err = .{ .message = try std.fmt.allocPrint(allocator, "natyv prepare: could not open NATYV_CORE_SRC ('{s}'): {s}", .{ natyv_core_src, @errorName(e) }) } };
    };
    defer core_dir.close(io);
    var core_assets = try core_dir.createDirPathOpen(io, "src/assets", .{});
    defer core_assets.close(io);

    var embed_line: []const u8 = "pub const data: ?[]const u8 = null;\n";
    if (file) |path| {
        const real_assets = assets_dir orelse return .{ .has_app_font = false, .err = .{ .message = try std.fmt.allocPrint(allocator, "natyv prepare: a font \"{s}\" is configured but no 'assets/' directory exists next to conf.natyv.json", .{path}) } };
        const bytes = real_assets.readFileAlloc(io, path, allocator, .unlimited) catch |e| {
            return .{ .has_app_font = false, .err = .{ .message = try std.fmt.allocPrint(allocator, "natyv prepare: could not read font 'assets/{s}': {s}", .{ path, @errorName(e) }) } };
        };
        var fonts_dir = try core_dir.createDirPathOpen(io, "src/assets/fonts", .{});
        defer fonts_dir.close(io);
        // Staged under one fixed name regardless of the source filename:
        // there is exactly one app font, and a stable name keeps the
        // generated @embedFile path from depending on what the developer
        // happened to call their file.
        const ext = std.fs.path.extension(path);
        const staged = try std.fmt.allocPrint(allocator, "app{s}", .{ext});
        try fonts_dir.writeFile(io, .{ .sub_path = staged, .data = bytes });
        embed_line = try std.fmt.allocPrint(allocator, "pub const data: ?[]const u8 = @embedFile(\"fonts/{s}\");\n", .{staged});
    }

    var out: std.ArrayList(u8) = .empty;
    try out.appendSlice(allocator, "// Code generated by natyv prepare. DO NOT EDIT.\n//\n// The app-wide default font, resolved from the `.ntss` `font` block and\n// conf.natyv.json's ui.font/ui.font_size (the stylesheet wins per field).\n// Null means natyv-core keeps its own bundled Inter / 16.0.\n\n");
    try out.appendSlice(allocator, embed_line);
    if (size) |pt| {
        try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "pub const point_size: ?f32 = {d};\n", .{pt}));
    } else {
        try out.appendSlice(allocator, "pub const point_size: ?f32 = null;\n");
    }
    try core_assets.writeFile(io, .{ .sub_path = "AppFontGenerated.zig", .data = out.items });
    return .{ .has_app_font = true, .err = null };
}

/// Stages conf.natyv.json's `icon` PNG into natyv-core so the system tray
/// can show it -- the *same* field the packaging step already turns into an
/// .icns/.ico/AppImage icon, deliberately not a tray-specific one. An app
/// that ships a tray already ships an app icon, and a second setting would
/// mostly be answered with the same file.
///
/// `icon_path` arrives already joined against the config file's own
/// directory (that is what `icon` is relative to -- see `Config.icon`),
/// unlike fonts and textures which live under `assets/`. Resolving it in
/// `cli/main.zig` keeps that difference in the one place that already knows
/// about `config_dir`.
///
/// A missing or unreadable file is a real error rather than a silent skip:
/// the developer named a file, so failing to find it is a mistake worth
/// surfacing, exactly as a missing font is.
fn stageTrayIcon(allocator: std.mem.Allocator, io: Io, icon_path: ?[]const u8, natyv_core_src: []const u8) !struct { has_tray_icon: bool, err: ?PrepareError } {
    const path = icon_path orelse return .{ .has_tray_icon = false, .err = null };

    const bytes = std.Io.Dir.cwd().readFileAlloc(io, path, allocator, .unlimited) catch |e| {
        return .{ .has_tray_icon = false, .err = .{ .message = try std.fmt.allocPrint(allocator, "natyv prepare: could not read icon '{s}': {s}", .{ path, @errorName(e) }) } };
    };

    var core_dir = std.Io.Dir.cwd().openDir(io, natyv_core_src, .{}) catch |e| {
        return .{ .has_tray_icon = false, .err = .{ .message = try std.fmt.allocPrint(allocator, "natyv prepare: could not open NATYV_CORE_SRC ('{s}'): {s}", .{ natyv_core_src, @errorName(e) }) } };
    };
    defer core_dir.close(io);
    var core_assets = try core_dir.createDirPathOpen(io, "src/assets", .{});
    defer core_assets.close(io);
    var tray_dir = try core_dir.createDirPathOpen(io, "src/assets/tray", .{});
    defer tray_dir.close(io);

    // One fixed staged name regardless of the source filename, same
    // reasoning as the app font's: there is exactly one icon, and a stable
    // name keeps the generated @embedFile path independent of whatever the
    // developer called their file.
    const ext = std.fs.path.extension(path);
    const staged = try std.fmt.allocPrint(allocator, "icon{s}", .{ext});
    try tray_dir.writeFile(io, .{ .sub_path = staged, .data = bytes });

    var out: std.ArrayList(u8) = .empty;
    try out.appendSlice(allocator, "// Code generated by natyv prepare. DO NOT EDIT.\n//\n// conf.natyv.json's `icon`, embedded for the system tray. natyv-core\n// decodes it through stb_image at tray-creation time -- see TrayDrain.zig.\n\n");
    try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "pub const data: ?[]const u8 = @embedFile(\"tray/{s}\");\n", .{staged}));
    try core_assets.writeFile(io, .{ .sub_path = "TrayIconGenerated.zig", .data = out.items });

    return .{ .has_tray_icon = true, .err = null };
}

fn stageWindowStyle(allocator: std.mem.Allocator, io: Io, window: ?Resolver.ResolvedWindow, natyv_core_src: []const u8) !struct { has_window_style: bool, err: ?PrepareError } {
    const win = window orelse return .{ .has_window_style = false, .err = null };
    // Any one field is enough to stage: a block that sets only `width`
    // must still reach natyv-core, which is why this checks all of them
    // rather than short-circuiting on the background alone.
    if (win.background_color == null and win.width == null and win.height == null) {
        return .{ .has_window_style = false, .err = null };
    }

    var core_dir = std.Io.Dir.cwd().openDir(io, natyv_core_src, .{}) catch |e| {
        return .{ .has_window_style = false, .err = .{ .message = try std.fmt.allocPrint(allocator, "natyv prepare: could not open NATYV_CORE_SRC ('{s}'): {s}", .{ natyv_core_src, @errorName(e) }) } };
    };
    defer core_dir.close(io);
    var assets_dir = try core_dir.createDirPathOpen(io, "src/assets", .{});
    defer assets_dir.close(io);

    var out: std.ArrayList(u8) = .empty;
    try out.appendSlice(allocator,
        \\// Code generated by natyv prepare. DO NOT EDIT.
        \\//
        \\// The `.ntss` reserved `window` block. Each field overrides its
        \\// conf.natyv.json counterpart -- styling wins over config. A field
        \\// the block did not set stays null, so the two sources compose
        \\// per-field rather than all-or-nothing.
        \\
        \\
    );

    // Colors are emitted as 0-255 channels rather than the resolver's
    // normalized floats: natyv-core hands these straight to
    // SDL_SetRenderDrawColor, which takes u8, so the rounding happens once
    // here instead of at every use.
    if (win.background_color) |bg| {
        try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "pub const background: ?[4]u8 = .{{ {d}, {d}, {d}, {d} }};\n", .{
            @as(u8, @intFromFloat(@round(bg.r * 255.0))),
            @as(u8, @intFromFloat(@round(bg.g * 255.0))),
            @as(u8, @intFromFloat(@round(bg.b * 255.0))),
            @as(u8, @intFromFloat(@round(bg.a * 255.0))),
        }));
    } else {
        try out.appendSlice(allocator, "pub const background: ?[4]u8 = null;\n");
    }

    if (win.width) |w| {
        try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "pub const width: ?u16 = {d};\n", .{w}));
    } else {
        try out.appendSlice(allocator, "pub const width: ?u16 = null;\n");
    }
    if (win.height) |h| {
        try out.appendSlice(allocator, try std.fmt.allocPrint(allocator, "pub const height: ?u16 = {d};\n", .{h}));
    } else {
        try out.appendSlice(allocator, "pub const height: ?u16 = null;\n");
    }

    try assets_dir.writeFile(io, .{ .sub_path = "WindowStyleGenerated.zig", .data = out.items });
    return .{ .has_window_style = true, .err = null };
}

const StyleTokensResult = struct {
    tokens: []const Resolver.ResolvedStyleToken,
    /// The reserved `window` block, if the stylesheet declared one. Kept
    /// apart from `tokens` by the resolver itself -- it never applies to a
    /// widget, it is consumed host-side.
    window: ?Resolver.ResolvedWindow = null,
    /// The reserved `font` block, same story as `window` above.
    font: ?Resolver.ResolvedFont = null,
    err: ?PrepareError,
};
...
fn findStyleTokens(allocator: std.mem.Allocator, io: Io, guest_dir: Io.Dir) !StyleTokensResult {
    ...
    var window: ?Resolver.ResolvedWindow = null;
    var font: ?Resolver.ResolvedFont = null;
    while (try walker.next(io)) |entry| {
        if (entry.kind != .file) continue;
    ...
    return .{ .tokens = tokens, .window = window, .font = font, .err = null };
}
...
/// Part 2 (codegen automation): a package-level ref= target name can
/// legitimately be shared across multiple composers -- mail-natyv's own
/// real, already-shipping `viewRoot` (ref'd by four separate view
/// composers -- ErrorView/FolderView/MessageDetailView/ComposeView --
/// sharing one package-level slot, exactly one view active at a time) is
/// what surfaced this, not a hypothetical. Each composer's own
/// `Codegen.generateGo` emission has no visibility into any *other*
/// composer reusing the same target_name (every one of them independently,
/// correctly produces its own entry), so deduplication has to happen here,
/// at the whole-app aggregation point -- the only place that ever sees all
/// of them together. Without this, `natyv_generated.go` would declare the
/// same `persistedGen_<name>`/`restoredGen_<name>` package-level var once
/// per composer that ref='d it -- a real Go "redeclared in this block"
/// error, not a cosmetic one. Keeps the first occurrence encountered
/// (first file processed, first composer within it) -- every duplicate
/// for the same target_name describes the identical real package-level
/// variable, so which one survives is immaterial.
fn dedupeByTargetName(comptime T: type, allocator: std.mem.Allocator, entries: []const T) ![]const T {
    var seen: std.StringHashMapUnmanaged(void) = .empty;
    defer seen.deinit(allocator);
    var out: std.ArrayList(T) = .empty;
    for (entries) |e| {
        if (seen.contains(e.target_name)) continue;
        try seen.put(allocator, e.target_name, {});
...
pub fn run(allocator: std.mem.Allocator, io: Io, guest_dir: Io.Dir, bindings: []const Config.BindingEntry, natyv_core_src: []const u8, mode: Mode, images_enabled: bool, assets_dir: ?Io.Dir, recycle_enabled: bool, config_font: ?[]const u8, config_font_size: ?f32, config_icon_path: ?[]const u8) !Outcome {
    ...
    // Same pass, same reasoning as texture staging above: `.ntss` data
    // natyv-core itself consumes has to be written into the core tree,
    // since styletokens_generated.go only ever reaches the guest.
    const font_result = try stageAppFont(allocator, io, assets_dir, style_result.font, config_font, config_font_size, natyv_core_src);
    if (font_result.err) |e| return .{
        .processed = 0,
        .binding_include_dirs = binding_include_dirs,
        .binding_lib_dirs = binding_lib_dirs,
        .binding_link = binding_link,
    ...
    const window_result = try stageWindowStyle(allocator, io, style_result.window, natyv_core_src);
    if (window_result.err) |e| return .{
        .processed = 0,
        .binding_include_dirs = binding_include_dirs,
        .binding_lib_dirs = binding_lib_dirs,
        .binding_link = binding_link,
    ...
    const tray_result = try stageTrayIcon(allocator, io, config_icon_path, natyv_core_src);
    if (tray_result.err) |e| return .{
        .processed = 0,
        .binding_include_dirs = binding_include_dirs,
        .binding_lib_dirs = binding_lib_dirs,
        .binding_link = binding_link,
    ...
    if (mode == .codegen_only) return .{
        ...
        .has_window_style = window_result.has_window_style,
        .has_app_font = font_result.has_app_font,
        .has_tray_icon = tray_result.has_tray_icon,
        .err = null,
    };
    ...
    if (recycle_enabled and processed > 0) {
        const deduped_refs = try dedupeByTargetName(Codegen.RefPersistEntry, allocator, gen_ref_entries.items);
        const deduped_structs = try dedupeByTargetName(Codegen.StructBackedSnapshotEntry, allocator, gen_struct_entries.items);
        const gen_src = try generateNatyvGeneratedGo(allocator, deduped_refs, deduped_structs);
...
test "transpiles a single .go.ntx file in place and validates cleanly" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &.{}, "", .full, false, null, false, null, null, null);
...
test "Part 2 Mechanism 1: recycle_enabled=true aggregates ref-persistence entries across two .ntx files into one natyv_generated.go" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &.{}, "", .full, false, null, true, null, null, null);
    try std.testing.expect(outcome.err == null);
...
test "Part 2 Mechanism 1: the same ref= target name shared across multiple composers dedupes to one declaration" {
    // Real, already-shipping shape this regression-tests directly:
    // mail-natyv's own viewRoot is ref='d by four separate view composers
    // (ErrorView/FolderView/MessageDetailView/ComposeView) sharing one
    // package-level slot, exactly one view active at a time. Each
    // composer's own generateGo call has no visibility into any other
    // composer reusing the same name, so without whole-app dedup this
    // would emit "var persistedGen_viewRoot"/"var restoredGen_viewRoot"
    // once per composer -- a real Go "redeclared in this block" compile
    // error, caught here at the natyv_generated.go text level instead.
    var tmp = std.testing.tmpDir(.{ .iterate = true });
    defer tmp.cleanup();
    const io = std.testing.io;
    ...
    try tmp.dir.writeFile(io, .{ .sub_path = "go.mod", .data = "module preptest\n\ngo 1.23\n" });
    try tmp.dir.writeFile(io, .{ .sub_path = "views.go.ntx", .data =
        \\package main
        \\
        \\expose ErrorView
        \\expose FolderView
        ...
        \\  <Container ref={&viewRoot}></Container>
        \\}
        \\
        \\func FolderView(parent widgets.Container) error {
        \\  <Container ref={&viewRoot}></Container>
        \\}
    });
    ...
    while (std.mem.indexOfPos(u8, gen, search_from, "var persistedGen_viewRoot ")) |idx| {
        decl_count += 1;
        search_from = idx + 1;
    }
...
test "texture-fill: referencing a texture with no assets/ directory is a clear error" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &.{}, "", .full, true, null, false, null, null, null);
    try std.testing.expect(outcome.err != null);
...
test "texture-fill: a real referenced asset is staged and its TextureID flows into styletokens_generated.go" {
    ...
    const cwd_path = try std.process.currentPathAlloc(io, allocator);
    const outcome = try run(allocator, io, tmp.dir, &.{}, cwd_path, .full, true, assets_tmp.dir, false, null, null, null);
    defer {
...
test "Stage 2.7: natyv bind runs as run()'s own first step, real fixture entry produces real Go output" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &bindings, cwd_path, .full, false, null, false, null, null, null);
    defer {
...
test "Stage 2.7: .codegen_only mode runs bind but skips .ntx transpile entirely" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &bindings, cwd_path, .codegen_only, false, null, false, null, null, null);
    defer {
        var bg = std.Io.Dir.cwd().openDir(io, "src/bindgen", .{}) catch unreachable;
        defer bg.close(io);
        bg.deleteFile(io, "fixture_bindings_generated.zig") catch {};
...
test "Stage 2.7: an empty bindings list is a real no-op, run() behaves exactly as before" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &.{}, "", .codegen_only, false, null, false, null, null, null);
    try std.testing.expect(outcome.err == null);
    try std.testing.expectEqual(@as(usize, 0), outcome.processed);
