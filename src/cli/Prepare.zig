...
/// real to call.
///
/// `struct_entries` isn't consumed yet -- tier-2/3 struct-backed
/// accessor wiring is a later stage of this same pass (needs new SDK
/// methods that don't exist yet); accepted now, alongside
/// `ref_entries`, so `run`'s own aggregation and this function's
/// signature don't need touching again once that stage lands.
