...
pub fn run(allocator: std.mem.Allocator, io: Io, guest_dir: Io.Dir, bindings: []const Config.BindingEntry, natyv_core_src: []const u8, mode: Mode, images_enabled: bool, assets_dir: ?Io.Dir, recycle_enabled: bool, config_font: ?[]const u8, config_font_size: ?f32) !Outcome {
...
test "transpiles a single .go.ntx file in place and validates cleanly" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &.{}, "", .full, false, null, false, null, null);
...
test "Part 2 Mechanism 1: recycle_enabled=true aggregates ref-persistence entries across two .ntx files into one natyv_generated.go" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &.{}, "", .full, false, null, true, null, null);
    try std.testing.expect(outcome.err == null);
...
test "texture-fill: referencing a texture with no assets/ directory is a clear error" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &.{}, "", .full, true, null, false, null, null);
    try std.testing.expect(outcome.err != null);
...
test "texture-fill: a real referenced asset is staged and its TextureID flows into styletokens_generated.go" {
    ...
    const cwd_path = try std.process.currentPathAlloc(io, allocator);
    const outcome = try run(allocator, io, tmp.dir, &.{}, cwd_path, .full, true, assets_tmp.dir, false, null, null);
    defer {
...
test "Stage 2.7: natyv bind runs as run()'s own first step, real fixture entry produces real Go output" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &bindings, cwd_path, .full, false, null, false, null, null);
    defer {
...
test "Stage 2.7: .codegen_only mode runs bind but skips .ntx transpile entirely" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &bindings, cwd_path, .codegen_only, false, null, false, null, null);
    defer {
        var bg = std.Io.Dir.cwd().openDir(io, "src/bindgen", .{}) catch unreachable;
        defer bg.close(io);
        bg.deleteFile(io, "fixture_bindings_generated.zig") catch {};
...
test "Stage 2.7: an empty bindings list is a real no-op, run() behaves exactly as before" {
    ...
    const outcome = try run(allocator, io, tmp.dir, &.{}, "", .codegen_only, false, null, false, null, null);
    try std.testing.expect(outcome.err == null);
    try std.testing.expectEqual(@as(usize, 0), outcome.processed);
