...
fn stageWindowStyle(allocator: std.mem.Allocator, io: Io, window: ?Resolver.ResolvedWindow, natyv_core_src: []const u8) !struct { has_window_style: bool, err: ?PrepareError } {
    ...
    const bg = win.background_color orelse return .{ .has_window_style = false, .err = null };
    ...
    // Emitted as 0-255 channels rather than the resolver's normalized
    // floats: natyv-core hands these straight to SDL_SetRenderDrawColor,
    // which takes u8, so converting here keeps the rounding in one place.
    const source = try std.fmt.allocPrint(allocator,
        \\// Code generated by natyv prepare. DO NOT EDIT.
        \\
        \\/// The `.ntss` `window` block's own backgroundColor. Overrides
        \\/// conf.natyv.json's `ui.background_color` -- styling wins over config.
        \\pub const background: ?[4]u8 = .{{ {d}, {d}, {d}, {d} }};
        \\
    , .{
    ...
    try assets_dir.writeFile(io, .{ .sub_path = "WindowStyleGenerated.zig", .data = source });
